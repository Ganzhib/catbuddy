...
├── packages/
│   ├── shared/                  # @catbuddy/shared — 协议与纯类型
│   ├── client/                  # @catbuddy/client — catbuddyClient + transport
│   ├── platform/                # @catbuddy/platform — bootstrap + REST/IPC API
│   └── ui/                      # @catbuddy/ui — React 应用主体
└── apps/
    ├── desktop/                 # @catbuddy/desktop — Electron 主进程 + 本地 Agent
    └── web/                     # @catbuddy/web — 纯 SPA，连 gateway
```
...
- [x] `catbuddyClient` 与传输层解耦，UI 通过 `onChat` 消费事件
...
| 会话/设置 API | `src/lib/api.ts` | 全部 `window.catbuddy.*` |
| 设置页 | `SettingsView.tsx` 等 | 直接调 IPC |
| 应用入口 | `App.tsx` | `restartApp()` 等桌面专属 |
| 构建 | `vite.config.ts` | 仅 electron 插件 |
| 类型入口 | `vite-env.d.ts` | `window.catbuddy` |

### 2.3 仓库内相关项目（收敛策略）

| 路径 | 角色 | 迁移后 |
|------|------|--------|
| `catbuddy/` | 主产品（本计划主体） | monorepo 根 |
...
- [ ] 根 `package.json`：`private: true`，scripts 转发到 `@catbuddy/desktop`
- [ ] 创建空包占位（每包 `package.json` + `tsconfig.json` + `src/index.ts` 导出占位）
- [ ] 保留现有 `src/`、`electron/` 直至 M4（**先不搬文件**，避免大爆炸）

**完成标准**: 根目录 `pnpm install` 成功；`pnpm --filter @catbuddy/desktop dev` 仍等价于当前 `pnpm dev`（通过 workspace 脚本指向现有 vite 入口）。
...
- [ ] `package.json`: `"name": "@catbuddy/shared"`, `"exports": { ".": "./src/index.ts" }`
- [ ] 全仓 `from '../../shared/types'` → `from '@catbuddy/shared'`

**`packages/client`**

- [ ] 迁入：
  - `src/lib/catbuddy-client.ts`
  - `src/lib/transport/**`
  - `src/lib/tool-traces.ts`（客户端映射依赖）
- [ ] 依赖 `@catbuddy/shared`
- [ ] 导出：`catbuddyClient`, `createCatbuddyClient`, `createAgentTransport`, transport 类型

**桌面适配**

- [ ] `catbuddy` 根或暂存 `src/lib` 改为 re-export（薄 shim，便于过渡）：

  ```ts
  export * from '@catbuddy/client'
  ```

- [ ] `tsconfig` paths: `@catbuddy/client` → `packages/client/src`
...
**完成标准**: 无 `catbuddy-client` 实现留在 `src/lib`（仅 re-export）；lint 通过；桌面对话正常。
...
- [ ] `create-platform.ts` — `hasCatbuddyIpc() ? ipc : http`
...
- [ ] `useSessions.ts`、`SettingsView.tsx`、`ThreadShell.tsx`：改用 `platform` 而非直接 `window.catbuddy`
- [ ] 桌面专属：`restartApp()` 保留在 `apps/desktop` 薄封装，不放进 `platform`

**完成标准**

- 桌面：会话列表、设置、发消息与 M2 一致
- Web（可用 `vite dev` 临时入口引 platform）：至少 bootstrap 不抛「IPC bridge not available」；能 `createCatbuddyClient` + 连 WS（gateway 未就绪时可显示连接错误 UI）
...
- [ ] 依赖：`@catbuddy/client`, `@catbuddy/platform`, `@catbuddy/shared`
- [ ] 平台能力通过 props 或 context 注入（如 `onRestartApp?: () => void` 仅 desktop 传入）

**`apps/desktop`**

- [ ] 迁入：`electron/**`, `templates/`, `skills/`, `electron/assets/`
- [ ] `vite.config.ts` — electron 插件，alias 指向 `@catbuddy/ui`
- [ ] `main.tsx` 在 `apps/desktop/src/main.tsx`：渲染 `@catbuddy/ui`
- [ ] `preload.cjs` 路径、`electron-builder.files` 更新为 monorepo 相对路径
- [ ] `package.json` name: `@catbuddy/desktop`

**构建**

- [ ] `shared` 类型仍被 electron 主进程引用 → `apps/desktop` 依赖 `@catbuddy/shared`
...
**目标**: 浏览器可访问的 catbuddy，连 gateway。

**`apps/web`**

- [ ] `vite.config.ts`：仅 `@vitejs/plugin-react`，**无** electron 插件
- [ ] `index.html` + `src/main.tsx` 挂载 `@catbuddy/ui`
- [ ] devServer.proxy：
  - `/api` → `http://127.0.0.1:8765`（或配置项）
  - `/webui` → gateway
  - WebSocket → `ws://127.0.0.1:8765/...`
- [ ] 环境变量：`VITE_GATEWAY_URL` 等
- [ ] `createCatbuddyClient({ transportMode: 'web' })` 或 `auto`
...
- [ ] CI（可选）：`pnpm -r lint`；`build:web`；`build:desktop` 矩阵（仓库根为 `nanobot` 时需在根 `.github` 配置 `working-directory: catbuddy`）
- [x] 清理 dead code（§2.4）：移除 `ipc-client`、空 `subagent.ts`、未使用 trace 辅助函数
- [x] 更新 `PROJECT_PLAN.md` 或归档
- [x] 版本号策略：`@catbuddy/*` 内部 `0.0.0` workspace 协议即可
...
| `src/lib/catbuddy-client.ts` | `packages/client/src/catbuddy-client.ts` |
...
2. `refactor: extract @catbuddy/shared and @catbuddy/client (M2)`  
3. `feat: @catbuddy/platform with ipc + http (M3)`  
4. `refactor: extract @catbuddy/ui and apps/desktop (M4)`  
