...
const Lightfall: React.FC<LightfallProps> = ({
  ...
  useEffect(() => {
    ...
    const mouseTargetRef = { x: 0, y: 0 }
    let lastTime = 0

    const onPointerMove = (e: PointerEvent) => {
      const rect = canvas.getBoundingClientRect()
      const scale = renderer.dpr || 1
      mouseTargetRef.x = (e.clientX - rect.left) * scale
      mouseTargetRef.y = (rect.height - (e.clientY - rect.top)) * scale
      if (mouseDampening <= 0) {
        uniforms.iMouse.value = [mouseTargetRef.x, mouseTargetRef.y]
      }
    }
    if (mouseInteraction) {
    ...
    const loop = (t: number) => {
      ...
      if (mouseDampening > 0) {
        ...
        cur[0] += (mouseTargetRef.x - cur[0]) * factor
        cur[1] += (mouseTargetRef.y - cur[1]) * factor
      } else {
        lastTime = t
      }
      if (!paused) {
