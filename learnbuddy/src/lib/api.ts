...
export async function listSessions(
  ...
  const sessions = await window.learnbuddy.listSessions()
...
export async function fetchWebuiThread(
  ...
  const session = await window.learnbuddy.getSession(key)
...
export async function updateSettings(
  ...
  if (update.model) await window.learnbuddy.setModel(update.model)
...
export async function updateProviderSettings(
  ...
  if (update.apiKey !== undefined) await window.learnbuddy.updateConfig(`${path}.apiKey`, update.apiKey)
  if (update.apiBase !== undefined) await window.learnbuddy.updateConfig(`${path}.apiBase`, update.apiBase)
  return fetchSettings(_token, _base)
}
