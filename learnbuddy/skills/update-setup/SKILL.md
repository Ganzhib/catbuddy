---
name: update-setup
description: One-time setup wizard for the learnbuddy upgrade skill. Triggers: setup update, configure update, 切设置更新, 初始化更新.
---
...
Use `exec` to run `learnbuddy --version`. Tell the user the current version.

Then collect install clues with `exec`. These commands are best-effort; if one fails,
keep going and show the useful output:

```
command -v learnbuddy || true
python -m pip show learnbuddy-ai || true
pipx list | sed -n '/learnbuddy-ai/,+3p' || true
uv tool list | sed -n '/learnbuddy-ai/,+3p' || true
```
...
answer, stop and ask the user to rerun this setup when they know how learnbuddy was
installed.
...
`question: "Where is your learnbuddy source checkout? Enter an absolute path or a path relative to this workspace:"`.
...
| Method | Command |
|--------|---------|
| uv | `uv tool install "learnbuddy-ai[EXTRAS]" --force` |
| pipx | `pipx install --force "learnbuddy-ai[EXTRAS]"` |
| pip | `python -m pip install --upgrade "learnbuddy-ai[EXTRAS]"` |
...
---
name: update
description: "Upgrade learnbuddy to the latest version. Triggers: upgrade learnbuddy, update learnbuddy, 升级learnbuddy, 更新learnbuddy."
---
...
4. Use `exec` to verify: `learnbuddy --version`
5. Tell the user the new version. Say: "Run `/restart` to restart learnbuddy and apply the update. If `/restart` is unavailable in this channel, restart the learnbuddy process manually."
```

## Step 5: Confirm

Tell the user: "Upgrade skill created. Say 'upgrade learnbuddy' when you want to update."
