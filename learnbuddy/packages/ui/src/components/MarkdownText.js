...
import { Suspense, lazy, memo, startTransition, useCallback, useEffect, useLayoutEffect, useRef, useState, } from "react";
...
const MemoizedMarkdownRenderer = memo(function MemoizedMarkdownRenderer({ source, className, highlightCode, }) {
    return (_jsx(LazyMarkdownRenderer, { className: className, highlightCode: highlightCode, children: source }));
});
const SHORT_STREAM_COMMIT_MS = 80;
const MEDIUM_STREAM_COMMIT_MS = 140;
const LONG_STREAM_COMMIT_MS = 220;
export function preloadMarkdownText() {
...
export function MarkdownText({ children, className, streaming = false, }) {
    ...
    return (_jsx(Suspense, { fallback: _jsx("div", { className: cn("whitespace-pre-wrap break-words leading-relaxed text-foreground/92", className), children: renderedSource }), children: _jsx(MemoizedMarkdownRenderer, { source: renderedSource, className: className, highlightCode: highlightCode }) }));
}
function useStreamingMarkdownSource(source, streaming) {
    ...
    const commitSource = useCallback((next, urgent) => {
        if (renderedSourceRef.current === next)
            return;
...
function streamingCommitDelay(length) {
    if (length > 24_000)
        return LONG_STREAM_COMMIT_MS;
    if (length > 8_000)
        return MEDIUM_STREAM_COMMIT_MS;
