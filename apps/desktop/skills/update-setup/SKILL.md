---
name: update-setup
description: One-time setup wizard for the catbuddy upgrade skill. Triggers: setup update, configure update, 切设置更新, 初始化更新.
---
...
Use `exec` to run `catbuddy --version`. Tell the user the current version.

Then collect install clues with `exec`. These commands are best-effort; if one fails,
keep going and show the useful output:

```
command -v catbuddy || true
python -m pip show catbuddy-ai || true
pipx list | sed -n '/catbuddy-ai/,+3p' || true
uv tool list | sed -n '/catbuddy-ai/,+3p' || true
```
...
answer, stop and ask the user to rerun this setup when they know how catbuddy was
installed.
...
`question: "Where is your catbuddy source checkout? Enter an absolute path or a path relative to this workspace:"`.
...
| Method | Command |
|--------|---------|
| uv | `uv tool install "catbuddy-ai[EXTRAS]" --force` |
| pipx | `pipx install --force "catbuddy-ai[EXTRAS]"` |
| pip | `python -m pip install --upgrade "catbuddy-ai[EXTRAS]"` |
...
---
name: update
description: "Upgrade catbuddy to the latest version. Triggers: upgrade catbuddy, update catbuddy, 升级catbuddy, 更新catbuddy."
---
...
4. Use `exec` to verify: `catbuddy --version`
5. Tell the user the new version. Say: "Run `/restart` to restart catbuddy and apply the update. If `/restart` is unavailable in this channel, restart the catbuddy process manually."
```

## Step 5: Confirm

Tell the user: "Upgrade skill created. Say 'upgrade catbuddy' when you want to update."
