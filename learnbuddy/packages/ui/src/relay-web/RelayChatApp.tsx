    ...
    <div className="flex h-full flex-col bg-[#0f0f10] text-zinc-100">
          ...
          >
            <label className="block space-y-1.5 text-sm">
              ...
              <input
                ...
                placeholder="http://localhost:5173/relay-api（开发默认）"
              />
            </label>
            <label className="block space-y-1.5 text-sm">
            ...
            <label className="block space-y-1.5 text-sm">
              <span className="text-zinc-400">Viewer Token（自选，配对后作 Bearer）</span>
              <input
      ...
      {connected ? null : (
        <footer className="shrink-0 px-4 pb-6 text-center text-[11px] text-zinc-600">
          先启动 relay-server 与桌面（RELAY_ENABLED=true），再打开此页。
        </footer>
      )}
    </div>
  );
