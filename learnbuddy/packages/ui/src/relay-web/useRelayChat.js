...
import { connectRelayViewer, pairRelayViewer, relayWsUrlFromHttp, resolveRelayHttpBase, sendRelayMessage, } from "@/lib/relay-api";
...
export function loadRelayWebPrefs() {
    return {
        ...
        viewerToken: localStorage.getItem(LS_TOKEN)?.trim()
...
function findStreamingAssistantIndex(messages) {
...
export function applyRelayInbound(messages, ev) {
    switch (ev.event) {
        case "user_inbound": {
...
export function useRelayChat() {
    ...
    const connect = useCallback(async (opts) => {
        ...
        try {
            ...
            const pair = await pairRelayViewer(httpBase, opts.pairingCode.trim().toUpperCase(), opts.viewerToken.trim());
            if (!pair.ok) {
    ...
    const send = useCallback(async (content) => {
        if (!config || !sessionKey || !content.trim())
            return;
        setSending(true);
        setError(null);
        ...
        try {
            const res = await sendRelayMessage(config.httpBase, sessionKey, config.viewerToken, text);
            if (!res.ok) {
...
//# sourceMappingURL=useRelayChat.js.map