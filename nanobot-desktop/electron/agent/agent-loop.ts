...
import type {
  InboundMessage, OutboundMessage, LLMMessage,
  ToolEvent, TurnCompleteData, SkillInfo,
} from '../../shared/types'
...
export class AgentLoop {
  ...
  constructor(opts: {
    ...
    this.context = new ContextBuilder(opts.workspace, { timezone: opts.timezone, disabledSkills: opts.disabledSkills })
  ...
  // ═══ 公开属性 ═══
  get uptime() { return Math.floor((Date.now() - this._startTime) / 1000) }
  get activeSessionCount() { return this._activeTasks.size }

  // ═══ 主入口 ═══
  async process(msg: InboundMessage, cbs?: StreamCallbacks): Promise<OutboundMessage | null> {
    ...
    while (ctx.state !== State.DONE) {
      ...
      const fn = (this as any)[handler] as ((ctx: TurnCtx, cbs?: StreamCallbacks) => Promise<string>)
      ...
      if (!next) throw new Error(`No transition from ${State[ctx.state]} on "${event}"`)
      ctx.state = next
    }
  ...
  private async _state_compact(ctx: TurnCtx): Promise<string> {
    const allMessages = this.sessions.getHistory(ctx.sessionKey, { maxMessages: 9999 })
    ...
    if (enabled && allMessages.length >= threshold) {
      ...
      // 同步等待压缩完成，再回答用户
      const summary = await this.consolidator.compactIdleSession(ctx.sessionKey, keepRecent)
      ctx.onToolProgress?.({ name: 'consolidator', status: 'completed' })
      ctx.onSystemMessage?.(summary
  ...
  private async _state_command(ctx: TurnCtx): Promise<string> {
    ...
    if (raw === '/model' || raw.startsWith('/model ')) {
      ...
      if (args.length > 0 && this.modelPresets[args[0]]) {
        ...
        const presets = Object.keys(this.modelPresets).join(', ') || '(none configured)'
        ctx.outbound = {
          channel: ctx.msg.channel, chatId: ctx.msg.chatId,
          content: `Current model: \`${this.model}\`\nAvailable presets: ${presets}\nUsage: /model <preset-name>`,
          media: [], metadata: {}, buttons: [],
        }
      }
      return 'shortcut'
    ...
    if (raw === '/compact') {
      const msgs = this.sessions.getHistory(ctx.sessionKey, { maxMessages: 9999 })
      if (msgs.length < 2) {
        ctx.outbound = { channel: ctx.msg.channel, chatId: ctx.msg.chatId, content: '(not enough messages to compact)', media: [], metadata: {}, buttons: [] }
        return 'shortcut'
      }
      ctx.onToolProgress?.({ name: 'consolidator', status: 'started' })
      try {
        const summary = await this.consolidator.compactIdleSession(ctx.sessionKey, Math.max(2, msgs.length - 2))
        ctx.outbound = {
          channel: ctx.msg.channel, chatId: ctx.msg.chatId,
          content: summary ? `Compacted. Summary:\n${summary.slice(0, 500)}` : 'Nothing to compact.',
          media: [], metadata: {}, buttons: [],
        }
      } catch (err: any) {
        ctx.outbound = { channel: ctx.msg.channel, chatId: ctx.msg.chatId, content: `Compact failed: ${err.message}`, media: [], metadata: {}, buttons: [] }
      }
      ctx.onToolProgress?.({ name: 'consolidator', status: 'completed' })
      return 'shortcut'
    }
  ...
  private async _state_build(ctx: TurnCtx): Promise<string> {
    ...
    const lastSummary = session.metadata?._last_summary as { text?: string; last_active?: string } | undefined
    if (lastSummary?.text) {
      sessionSummary = `Previous conversation summary (last active ${lastSummary.last_active ?? 'unknown'}):\n${lastSummary.text}`
    }
  ...
  private async _state_run(ctx: TurnCtx, cbs?: StreamCallbacks): Promise<string> {
    // 持久化用户消息
    this.sessions.addMessage(ctx.sessionKey, {
      role: 'user', content: ctx.msg.content,
      media: ctx.msg.media, timestamp: new Date().toISOString(),
    })
  ...
  setProvider(provider: LLMProvider) {
    ...
    this.consolidator.setProvider(provider, this.model, this.contextWindowTokens)
  }
