...
export function sniffImageMime(bytes) {
    ...
    if (bytes.length >= 6) {
        const g1 = bytes[0] === 0x47 && bytes[1] === 0x49 && bytes[2] === 0x46 &&
...
function computeScaledDims(srcW, srcH, maxEdge) {
...
async function normalize(file, sourceMime) {
...
export async function encodeImageInWorker(input) {
    const { id, file } = input;
    ...
    if (!sniffed)
        return { id, ok: false, reason: "magic_mismatch" };
...
//# sourceMappingURL=imageEncode.worker.js.map