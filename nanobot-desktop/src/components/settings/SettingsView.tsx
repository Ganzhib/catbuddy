...
function CompactionSettings() {
  ...
  const save = async () => {
    await window.nanobot.updateConfig('agents.defaults.autoCompact', { enabled, threshold })
  }

  return (
    <section>
      <SettingsSectionTitle>{t("settings.rows.compaction")}</SettingsSectionTitle>
      <SettingsGroup>
        <SettingsRow
          title={t("settings.rows.compactionEnable")}
          description={t("settings.help.compactionEnable")}
        >
          <button
            onClick={() => { setEnabled(!enabled); save() }}
            className={`relative inline-flex h-6 w-10 items-center rounded-full transition-colors ${enabled ? 'bg-blue-600' : 'bg-gray-600'}`}
          >
        ...
        <SettingsRow
          title={t("settings.rows.compactionThreshold")}
          description={t("settings.help.compactionThreshold")}
        >
          <div className="flex items-center gap-0">
            <button
              onClick={() => { if (threshold > 4) { setThreshold(threshold - 1); save() } }}
              disabled={threshold <= 4}
              className="h-7 w-6 text-gray-400  disabled:opacity-20 text-sm"
            >−</button>
            <span className="w-10 h-7 text-center text-sm text-gray-800 tabular-nums border border-gray-700 rounded flex items-center justify-center">{threshold}</span>
            <button
              onClick={() => { if (threshold < 500) { setThreshold(threshold + 1); save() } }}
              disabled={threshold >= 500}
              className="h-7 w-6 text-gray-400  disabled:opacity-20 text-sm"
            >+</button>
          </div>
        </SettingsRow>
