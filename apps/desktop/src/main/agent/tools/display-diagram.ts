...
export function createDisplayDiagramTool(ctx: ToolContext): Tool {
  return {
    ...
    definition: {
      ...
      function: {
        name: 'display_diagram',
        description: `Create and display a NEW Draw.io diagram.

CRITICAL: Generate ONLY bare <mxCell> elements — NO wrapper tags (<mxfile>, <mxGraphModel>, <root>). Wrapper and root cells (id="0", id="1") are added automatically.

Layout: x=0~800, y=0~600, start x=40/y=40, gap 150-200px. Container max: 700x550.

Cell Rules:
- Unique sequential IDs from "2". Top-level parent="1".
- mxCell must be siblings, never nested.
- Escape chars: &lt; &gt; &amp; &quot;

Edge Rules:
- Always set exitX/Y + entryX/Y. Stagger parallel edges (0.3/0.7).
- Route around obstacles with <Array as="points"><mxPoint .../>.
- Natural connection points, never corners.

For cloud diagrams: call get_shape_library first.`,
        parameters: {
          type: 'object',
          properties: {
            xml: {
              type: 'string',
              minLength: 1,
              description: 'Bare <mxCell> elements ONLY — NO wrapper tags. Wrapper is auto-added. Each mxCell must have unique id (from "2"), valid parent, and mxGeometry.',
            },
            path: {
              type: 'string',
