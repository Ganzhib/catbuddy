...
export function toolProgressKey(event) {
    if (event.call_id)
        return event.call_id;
    return event.name ? `name:${event.name}` : "unknown";
}
function formatArgs(arguments_) {
    if (arguments_ == null)
        return "";
...
export function toolCallUiStatus(event) {
...
export function toolCallTitle(event) {
...
export function formatToolArgumentsBlock(arguments_) {
    if (arguments_ == null)
        return null;
    if (typeof arguments_ === "string") {
        const trimmed = arguments_.trim();
...
export function toolCallDetailText(event) {
...
export function sortedToolProgressEntries(toolProgress) {
...
export function linesFromToolProgress(toolProgress) {
    if (!toolProgress)
        return [];
    return Object.values(toolProgress).map(formatToolProgressLine);
}
export function upsertToolProgress(existing, incoming) {
...
export function toolProgressFromBackendEvent(data) {
    const phase = data.status === "started"
        ? "start"
        : data.status === "error"
            ? "error"
            : "end";
...
//# sourceMappingURL=tool-traces.js.map