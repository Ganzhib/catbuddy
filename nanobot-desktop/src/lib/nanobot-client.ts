/**
 * IPC-based NanobotClient — 保持与原版 WebSocket 客户端相同的公开 API
 * 内部使用 window.nanobot IPC bridge，替代 WebSocket 通信
 */
import type {
...
export class NanobotClient {
    ...
    this._unsubs = [
      ...
      api.onStreamEnd(({ streamId, resuming: _resuming }) => {
        ...
        this._dispatch(this._activeChatId, {
          event: 'turn_end',
          chat_id: this._activeChatId,
          latency_ms: this._latencyMs ?? undefined,
        })
        // 通知 goal 状态
        this._dispatch(this._activeChatId, {
          event: 'goal_status',
  ...
  sendMessage(
    ...
    window.nanobot.sendMessage(chatId, content, mediaUrls)
  }
