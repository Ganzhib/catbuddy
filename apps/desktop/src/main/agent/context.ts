...
// Bundled main lives in dist-electron/ → ../templates is the app templates root.
...
function loadTemplate(relativePath: string): HandlebarsTemplateDelegate {
  ...
  // 仅处理简单 Jinja 块标签；带 == / or 的条件请在模板里用 Handlebars 布尔变量（见 _channelFlags）
...
export class ContextBuilder {
  ...
  private _channelFlags(channel: string) {
  ...
  private _renderIdentity(channel: string): string {
    ...
    const platformPolicy = this._templates.platformPolicy
      ? this._templates.platformPolicy({ isWindows })
      : ''
