...
import { isDevAuthBypass, isViewerLoginRequired } from './auth-policy.js'
...
export class AuthService {
  ...
  async resolveViewerToken(authorization?: string): Promise<string> {
    ...
    if (!bearer) {
      if (isViewerLoginRequired()) unauthorized(this.loginRequiredBody())
      unauthorized()
    }
  ...
  async requestEmailCode(email: string): Promise<{ ok: true; expiresIn: number }> {
    ...
    if (!pending) unauthorized('no_pending_registration')
  ...
  private async issueViewerToken(
    ...
    const access_token = jwt.sign(
      { sub: email, role: 'viewer', typ: 'gateway_viewer' } satisfies JwtViewerPayload,
  ...
  async registerTokenFromJwt(token: string): Promise<boolean> {
    try {
      const payload = jwt.verify(token, gatewayEnv.jwtSecret) as JwtViewerPayload
      const typ = payload.typ
      if ((typ !== 'gateway_viewer' && typ !== 'relay_viewer') || payload.role !== 'viewer') {
        return false
      }
      const email = payload.sub?.trim().toLowerCase()
      this.gateway.registerViewerToken(token, email?.includes('@') ? email : undefined)
      return true
    } catch {
      return false
    }
