...
本文件为 Codex 提供代码仓库的上下文指引。

> 🐱 **新队友？** → 先看 [Codex 上手指南](docs/CLAUDE_CODE_ONBOARDING.md)，5 分钟上手。
...
在 Codex 中使用 `/skill-name` 快速获得特定子系统的上下文：
...
- Codex 生成的 commit 会附加 `Co-Authored-By: Codex <noreply@anthropic.com>` 尾注

## .Codex/ 配置说明

本项目的 `.Codex/` 目录做了完整配置以最大化 Codex 效能：
